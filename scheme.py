...
def eval_all(expressions, env, is_tail = False):
    ...
    while expressions != nil:
        if expressions.rest == nil:
            return scheme_eval(expressions.first, env, is_tail)
...
def do_and_form(expressions, env):
    ...
    while expressions != nil:
        if expressions.rest == nil:
            exp = scheme_eval(expressions.first, env, True)
        else:
...
def optimize_tail_calls(prior_eval_function):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        while isinstance(result, Thunk):
            ...
            result = prior_eval_function(expr, env)
        return result
        # END
